...
public class SwaggerConfig {                                    
    ...
    public Docket api() { 
        return new Docket(DocumentationType.SWAGGER_2) 
          .select()                                  
          .apis(RequestHandlerSelectors.basePackage("com.skipthedishes.resource"))  
          .paths(regex("/api.*"))                     
          .build()
          .apiInfo(metaData());
    }
	...
	private ApiInfo metaData() {
		ApiInfo apiInfo = new ApiInfo("Skip the dishes test REST API", "Skip the dishes REST API for cousine", "1.0",
				"Terms of service",
